...
namespace Capstone.Application.Module.Account.QueryHandle
{
    public class ForgotPasswordQueryHandle : IRequestHandler<ForgotPasswordQuery, ResponseMediator>
    {
        ...
        public async Task<ResponseMediator> Handle(ForgotPasswordQuery request, CancellationToken cancellationToken)
        {
            ...
            if(ac == null)
                return new ResponseMediator(false, "This email was not registed", null);

            var code =  await _jwtService.generatejwttokentw(ac, 1);
            ...
            (bool isSuccess, string errorMessage) =  await _emailService.SendEmailAsync(body, request.Email);

            if(!isSuccess)
                return new ResponseMediator(false, errorMessage, null);
